...
class KeepPatch(Corruption):
    def __init__(self, size_percent=0.5, im_size=64):
        ...
        self.size_patch = (int(self.size_percent * im_size),
                           int(self.size_percent * im_size))
    ...
    def __call__(self, x, device='cpu', theta=None, seed=None):
        assert (self.im_size is not None)
        fx = x.clone()

        if theta is None:
            mask = self.sample_theta(im_shape=x.shape, seed=seed)
            mask = torch.tensor(mask, dtype=torch.bool, device=device, requires_grad=False)
        else:
            mask = theta
...
class RemovePixelDark(Corruption):
    ...
    """Randomly removes pixels, replacing them with black pixels.
    """
